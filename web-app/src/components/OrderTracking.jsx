        ...
        <main className="mx-auto w-full max-w-4xl px-4 py-8 pb-24 font-sans text-slate-900">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                ...
                <div className="space-y-6">
                    ...
                    {(order.tracking?.courierName || order.tracking?.awbNumber) && (
                        <div className="rounded-3xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-slate-50 p-6">
                            <h3 className="mb-4 flex items-center gap-2 text-sm font-black tracking-wider text-slate-500 uppercase">
                                <Truck size={16} /> Courier Info
                            </h3>

                            {/* Courier Name + Status Badge */}
                            <div className="mb-3 flex flex-wrap items-center gap-2">
                                <p className="text-base font-black text-slate-900">
                                    {order.tracking.courierName || 'Standard Partner'}
                                </p>
                                <span
                                    className={`flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-extrabold tracking-widest uppercase ${
                                        order.status === 'SHIPPED'
                                            ? 'border-indigo-200 bg-indigo-100 text-indigo-700'
                                            : order.status === 'DELIVERED' || order.status === 'PROFIT_CREDITED'
                            ...
                            {order.tracking.trackingUrl && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-white py-2.5 text-xs font-bold text-indigo-600 shadow-sm ring-1 ring-indigo-100 transition-colors hover:bg-indigo-50"
                                >
