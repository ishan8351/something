...
const BeautifulDescription = ({ rawHtml }) => {
    const parsedData = useMemo(() => {
        ...
        const description = extract(
            /Description\s*:-([\s\S]*?)(?=Key Features\s*:-|Ideal For\s*:-|Specifications\s*:-|Keywords\s*:-|Dimension\s*:-|$)/i
        );
        const featuresRaw = extract(
            /Key Features\s*:-([\s\S]*?)(?=Ideal For\s*:-|Specifications\s*:-|Keywords\s*:-|Dimension\s*:-|$)/i
        );
        const idealForRaw = extract(
            /Ideal For\s*:-([\s\S]*?)(?=Specifications\s*:-|Keywords\s*:-|Dimension\s*:-|$)/i
        );
        const specsRaw = extract(
            /Specifications\s*:-([\s\S]*?)(?=Keywords\s*:-|Dimension\s*:-|$)/i
        );
